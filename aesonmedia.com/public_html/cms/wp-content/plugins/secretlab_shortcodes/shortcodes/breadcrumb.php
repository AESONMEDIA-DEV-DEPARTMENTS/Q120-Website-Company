	...
	if ( function_exists( 'woocommerce_breadcrumb' ) && ( is_woocommerce() || is_shop() ) ) {
		...
		if ( is_front_page() ) {
			...
			if ( is_home() ) {
				...
				$output .= $content . sprintf( $category, '<span class="archive-text">', '</span>' , $after ) .  get_the_archive_title() . $after;
			} elseif ( is_search() ) {
				$output .= $content . sprintf( $search, '<span class="search-text">', '</span>' . get_search_query() ) .  $after; // WPCS: XSS OK.
				...
				$output .= $content . esc_html( get_the_time( __( 'd', 'ssc' ) ) ) .  $after; // WPCS: XSS OK.
			} elseif ( is_month() ) {
				$output .= sprintf( $link, esc_url( get_year_link( get_the_time( __( 'Y', 'ssc' ) ) ) ), esc_html( get_the_time( __( 'Y', 'ssc' ) ) ) ) ; // WPCS: XSS OK.
				$output .= $content . esc_html( get_the_time( __( 'F', 'ssc' ) ) ) . $after; // WPCS: XSS OK.
			} elseif ( is_year() ) {
				$output .= $content . esc_html( get_the_time( __( 'Y', 'ssc' ) ) ) .  $after; // WPCS: XSS OK.
				...
				if ( get_post_type() != 'post' ) {
					...
					$output .= $content . esc_html( get_the_title() ) . $after; // WPCS: XSS OK
				}
				else {
				...
				$post_type = get_post_type_object( get_post_type() );
				$output .= isset( $post_type->labels->singular_name ) ? $content. esc_html( $post_type->labels->singular_name ) .  $after: '';  // WPCS: XSS OK.
				if ( null === $post_type && $kc_action = filter_input( INPUT_POST, 'kc_action' ) ) {
					if ( 'live-editor' === $kc_action  ) {
						if( isset( $_POST['ID'] ) ){
							$output .= $content . esc_attr( get_the_title( $_POST['ID'] ) ) . $after;
						} else {
							$output .= $content . esc_html__( 'Live Editor', 'ssc' ) . $after;
						}
					}
				}
				...
				$output .= $content. esc_html( get_the_title() ) . $after; // WPCS: XSS O
			} elseif ( is_page() && $post->post_parent ) {
				...
				$output .= $content . sprintf( $tag, '<span class="tag-text">', '</span>' ) . get_the_archive_title() . $after; // WPCS: XSS OK
			} elseif ( is_author() ) {
				global $author;
				$userdata = get_userdata( $author );
				$output .= $content . sprintf( $title_author, '<span class="author-text">', '</span>' . $userdata->display_name ) .  $after; // WPCS: XSS
			} elseif ( is_404() ) {
			...
			if ( get_query_var( 'paged' ) ) {
				...
				$output .= sprintf( esc_html__( 'Page %s', 'ssc' ), absint( max( $paged, $page ) ) );
				if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) {
					$output .= ')';
				}
			}
