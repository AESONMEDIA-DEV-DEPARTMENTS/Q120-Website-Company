...
function ssc_button_shortcode($atts, $content = null) {
    ...
    if ( 'yes' === $show_icon ) {
	    ...
	    if ( $icon_position == 'left' ) {
		    $output .= '<span class="svg-icon">' . ssc_process_svg( $svg_icon ) . '</span> '. esc_html( $text_title ) ;
	    } else {
		    $output .= esc_html( $text_title ) . ' <span class="svg-icon">' . ssc_process_svg( $svg_icon ) . '</span>';
	    }
    } else {
