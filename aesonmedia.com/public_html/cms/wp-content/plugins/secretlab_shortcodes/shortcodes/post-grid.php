...
//			        'text' => esc_html__( 'Text', 'ssc' ),
                    ...
                    array(
                        ...
                        'type' => 'text',
                        'value' => '20',
                        'description' => esc_html__( 'Set 0 if you dont want to display description', 'ssc' ),
                    ),
	                array(
		                'name' => 'limit_lines',
                        ...
                        'options' => array(
                            array(
                                ...
                                'Title' => array(
	                                ...
	                                array('property' => 'float', 'label' => esc_html__( 'Float', 'ssc' ), 'selector' => '.grid-item .title'),
	                                array('property' => 'display', 'label' => esc_html__( 'Display', 'ssc' ), 'selector' => '.grid-item .title'),
    ...
    if ( $the_query->have_posts() ) {
	    ...
	    */
        ...
        foreach ( $list_posts as $item ) {
            ...
            $formatico = '';
            if ($show_thumb === 'yes') {
                if ($template === '2' || $template === '3' || $template === '5' || $template === '6' || $template === '9' || $template === '12' || '13' === $template || '21' === $template ) {
            ...
            if ($show_date === 'yes' || $show_author === 'yes' || $show_category === 'yes' || $show_comment === 'yes') {
                if ($show_date === 'yes') {
                	if ( empty( $date_format ) ) {
		                ...
		                $date = '<span class="date"><i class="far fa-clock"></i> <time class="entry-date" datetime="' . get_the_date('c', $item->ID) . '">' . get_the_time('F j, Y', $item->ID) . '</time></span> <span class="updated">'. get_the_modified_time( $format, $item->ID).'</span>';
		                $date2 = '<span class="date"><i class="far fa-clock"></i> <time class="entry-date" datetime="' . get_the_date('c', $item->ID) . '">' . get_the_time('M j, Y', $item->ID) . '</time></span> <span class="updated">'. get_the_modified_time( $format, $item->ID).'</span>';
            ...
            if( !post_password_required( $item->ID ) ) {
                if ($limit_words > 0) {
                    if ($excerpt_type === 'content') {
	                    if (!empty(wp_trim_words($item->post_content, $limit_words))) {
		                    $myexcept = '<p>' . wp_trim_words(preg_replace( '~\[[^\]]+\]~', '', $item->post_content ), $limit_words) . '</p>';
	                    }
