	...
	public function import_sample_data() {
		...
		if ( ( isset( $_POST['import_data'] ) && $_POST['import_data'] == 1 ) || $_POST['op'] == 'import_sample_data' ) {
			...
			if ( ! class_exists( 'WP_Import' ) ) { // if WP importer doesn't exist
				$wp_import = dirname( __FILE__ ) . '/modules/wordpress-importer/secretlab-importer.php';
				include $wp_import;
			}
...
//					$this->upload_ipmort_file( 'demo_data.xml' );
			...
			// change parent URI to local URI in Post Content Visual Composer data
			$site_url = site_url();
			$posts    = get_posts( array( 'posts_per_page' => - 1, 'numberposts' => - 1, 'post_type' => 'any' ) );
			foreach ( $posts as $p ) {
				$post_content = preg_replace( '/url.http.3A.2F.2F(secpl1|secpl2|secpl3).secretlab.pw.2F/',
...
//			$theme_type = filter_input( INPUT_POST, 'theme_type' );
	...
	public function welcome_notice() {
		...
		if ( count( $init_msgs ) > 0 ) {
			$init_msg = '<div id="message" class="notice notice-warning is-dismissible"><p>' . implode( $init_msgs,
					'<br>' ) . '</p></div>';
		} else {
			$init_msg = '';
		}
...
if ( ! in_array( 'wordpress-importer.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
	$modules[] = 'wordpress-importer';
}

if ( is_array( $modules ) ) {
	foreach ( $modules as $module ) {
		if ( file_exists( dirname( __FILE__ ) . '/modules/' . $module . '/' . $module . '.php' ) ) {
