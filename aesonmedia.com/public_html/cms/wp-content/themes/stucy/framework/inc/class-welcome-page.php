...
class Atiframebuilder_Welcome {
		...
		wp_localize_script(
			...
			array(
				...
				'import_start_failed' => esc_html__( 'XML file reading error occurred - check its existence',
					'stucy' ),
				'emptyInput'          => esc_html__( 'Please select a file.', 'stucy' ),
				'noAttachments'       => esc_html__( 'There were no attachment files found in the import file.',
					'stucy' ),
				'parsing'             => esc_html__( 'Parsing the file.', 'stucy' ),
				'importing'           => esc_html__( 'Importing file ', 'stucy' ),
				'progress'            => esc_html__( 'Overall progress: ', 'stucy' ),
				'retrying'            => esc_html__( 'An error occurred. In 5 seconds, retrying file ',
					'stucy' ),
				'done'                => esc_html__( 'All done!', 'stucy' ),
    ...
    <a href="?page=welcome" class="nav-tab ' . $active_class . '">' . esc_html__( 'Installation', 'stucy' ) . '</a>
    <a href="?page=welcomedocs" class="nav-tab">' . esc_html__( 'Documentation', 'stucy' ) . '</a>
</h2>
</div>';
		...
		echo '<br><h1>', esc_html__( 'Welcome to SecretLab Theme', 'stucy' ), '</h1>
...
<div class="tab-content" >';
		...
		echo '<div role="tabpanel" class="tab-pane fade in active" id="firsttab"', $this->hide(), '>
            ...
            <div class="row">
                ...
                <h2>1. ', esc_html__( 'Automatic installation can take 2-8 min', 'stucy' ), '</h2>
                <div  id="theme_setup_box">
					...
					<div id="theme_setup_summary">
		                ...
		                <div id="theme_setup_control">
						 <div id="theme_type">
							 <span>', esc_html__( 'Select Theme version', 'stucy' ), '</span>
		...
		echo '
			...
			'stucy' ), '</b></p>
                                ...
                                <div id="import_attachment_data">
	                                ...
	                                <p>', esc_attr__( 'Attribute uploaded images to:', 'stucy' ), '<br/>
			...
			'stucy' ), wp_dropdown_users( array( 'echo' => false ) ) .

		                          '<p><input type="checkbox" checked="checked" name="delay" />&nbsp;', esc_attr__( 'Delay file requests by at least five seconds.',
			'stucy' ), '&nbsp;<a href="#" title="', esc_attr__( 'This delay can be useful to mitigate hosts that throttle traffic when too many requests are detected from an IP address and mistaken for a DDOS attack.',
			'stucy' ), '"><span class="dashicons dashicons-editor-help"></span></a></p> 									
                                </div>
		                        <div class="welcome_install">
			...
			'stucy' ), '</a></div>
						...
						<h2 class="second">', esc_html__( 'Log of installation', 'stucy' ), '</h2>
						<div class="', $install_log_class, '">
							   <div id="theme_setup_result"></div>
						</div>
                </div>
                <div class="col-md-6 col-sm-12">
                	<h2>', esc_html__( '2. Setup your theme', 'stucy' ), '</h2>
					<div class="helpful">
						<ol>
			...
			'stucy' ), '</a> ', esc_html__( '(required Redux Framework)',
			'stucy' ), '</li>
						<li><a href="admin.php?page=_sl_theme_options&tab=8">', esc_html__( 'Typography (required Redux Framework)', 'stucy' ), '</a></li>
		...
		echo '
                <div class="col-md-4 col-sm-12">
                    <h2 class="second">', esc_html__( 'Support', 'stucy' ), '</h2>
                    <div class="inform">
			...
			'stucy' ), '</a>
                    <p>', esc_attr__( 'Response time is 24 hours', 'stucy' ), '</p>
...
<div role="tabpanel" class="tab-pane fade" id="secondtab"', $this->hide(), '>
	<div class="row">
			...
			'stucy' ), '</p>
                                <p>', esc_attr__( 'Details about manual installations process: ', 'stucy' ), '<a href="http://secretlab.pw/doc/atiframe/#line4" target="_blank">http://secretlab.pw/doc/atiframe/#line4</a></p>
								<div id="progressBar2" class="progressBar"><div  class="progress_bar"></div><div class="progress_count"></div></div>';
		echo '
								 <div id="theme_type">
									 <span>', esc_attr__( 'Select Theme version', 'stucy' ), '</span>
		...
		echo '<li><span>1</span> <a target="_blank" href="themes.php?page=install-required-plugins&plugin_status=install">', esc_attr__( 'Begin installing plugins',
			'stucy' ), '</a> <a id="manual_install_plugins" class="manual_install manual_btn" href="#">', esc_attr__( 'Do It',
			'stucy' ), '</a></li>';
		...
		echo '<li><span>2</span> <a target="_blank" href="themes.php?page=install-required-plugins&plugin_status=activate">', esc_attr__( 'Begin activating plugins',
			'stucy' ), '</a> <a id="manual_activate_plugins" class="manual_install manual_btn" href="#">', esc_attr__( 'Do It',
			'stucy' ), '</a></li>';
		...
		echo '<li><span>5</span> <a target="_blank" href="admin.php?import=wordpress">', esc_attr__( 'Import Dummy Data (Skip it, if you dont need demo-data)',
			'stucy' ), '</a> ', esc_attr__( ' If it got error, you can run it again 2-6 times (at slow webhosting) until you get a success message',
			'stucy' ), ' <a id="manual_import_sample_data" class="manual_install manual_btn" href="#">', esc_attr__( 'Do It',
			'stucy' ), '</a></li>';

		echo '<li><span>6</span> <a target="_blank" href="admin.php?page=revslider">', esc_attr__( 'Import Sliders',
			'stucy' ), '</a> <a id="manual_set_sliders" class="manual_install manual_btn" href="#">', esc_attr__( 'Do It',
			'stucy' ), '</a></li>';

		echo '<li><span>7</span> <a target="_blank" href="admin.php?page=_sl_theme_options&tab=14">', esc_attr__( 'Import Theme Option Set',
			'stucy' ), '</a> <a id="manual_set_theme_options" class="manual_install manual_btn" href="#">', esc_attr__( 'Do It',
			'stucy' ), '</a></li>';

		echo '<li><span>8</span> <a target="_blank" href="admin.php?page=caldera-forms">', esc_attr__( 'Import Caldera Forms',
			'stucy' ), '</a> <a id="manual_import_caldera_forms" class="manual_install manual_btn" href="#">', esc_attr__( 'Do It',
			'stucy' ), '</a></li>';

		echo '<li><span>9</span> <a target="_blank" href="tools.php?page=widget-importer-exporter">', esc_attr__( 'Import Widgets',
			'stucy' ), '</a> <a id="manual_import_widgets" class="manual_install manual_btn" href="#">', esc_attr__( 'Do It',
			'stucy' ), '</a></li>';

		echo '<li><span>10</span> <a href="admin.php?page=_sl_theme_options&atiframebuilderfrom=welcome">', esc_attr__( 'Go to Theme Options page to finish',
			'stucy' ), '</a></li>';
		echo '</ol>';
		echo '</div></div>';

		echo '<div class="col-md-6 col-sm-12 ">
		<h2 class="second">', esc_attr__( 'Log of installation', 'stucy' ), '</h2>
