...
class Atiframebuilder_Header {
	...
	public function __construct() {
		...
		add_action( 'atiframebuilder_header_scripts', array( $this, 'header_js' ) );
	...
	public static function set_header_sidebar_layout() {
		...
		if ( class_exists( 'ReduxFrameworkPlugin' ) && class_exists( 'KingComposer' )) {
			...
			} elseif ( is_active_sidebar( '_default_left_sidebar') ) {
				echo '<div class="col-lg-3 col-md-12 col-sm-12 widget-area left_sb animated slideInLeft">
                        <div>';
				dynamic_sidebar( '_default_left_sidebar' );
				echo '</div>
	...
	public static function set_header_widget() {
		...
		if ( isset( $secretlab['header_widget'] ) && class_exists( 'custom_post_widget' ) ) {
			...
			if ( $atiframebuilder_layout[ $type . $prefix . 'header_widget' ] !== - 1 ) {
				...
				if ( ! empty( $styles['styles'] ) ) {
					...
					self::$header_widget = preg_replace( '@<(script|style)[^>]*?>.*?</\\1>@si', '', $header );
				} else {
	...
	public static function header_layout() {
		...
		if ( isset( $secretlab['header_widget'] ) && class_exists( 'custom_post_widget' ) ) {
            ...
            <div class="entry-meta">';
							...
							edit_post_link( esc_html__( 'Edit', 'stucy' ),
						...
						if ( is_search() ) {
							...
							printf( esc_html__( 'Search Results for: %s', 'stucy' ), get_search_query() );
							echo '</h1>';
							echo '</header>';
						}
						if ( is_day() ) {
							echo '<header class="custblog">';
							echo '<h1 class="archive-title">';
							printf( esc_html__( 'Daily Archives: %s', 'stucy' ), get_the_date() );
							echo '</h1>';
							echo '</header>';
						}
						if ( is_month() ) {
							echo '<header class="custblog">';
							echo '<h1 class="archive-title">';
							printf( esc_html__( 'Monthly Archives: %s', 'stucy' ),
								get_the_date( _x( 'F Y', 'monthly archives date format', 'stucy' ) ) );
							echo '</h1>';
							echo '</header>';
						}
						if ( is_year() ) {
							echo '<header class="custblog">';
							echo '<h1 class="archive-title">';
							printf( esc_html__( 'Yearly Archives: %s', 'stucy' ),
								get_the_date( _x( 'Y', 'yearly archives date format', 'stucy' ) ) );
							echo '</h1>';
							echo '</header>';
						}
						if ( is_404() ) {
							echo '<header class="custblog">
                       <h1>', esc_html__( '404', 'stucy' ), '</h1>
                    </header>
                    </div>';
						}
